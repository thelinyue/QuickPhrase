...
public class CategoryDeletionTests
{
    ...
    public async Task CascadeCategoryDelete_ReloadsOnlyAfterDeleteSucceeds()
    {
        ...
        var rootPhrase = new Phrase(Guid.NewGuid(), "欢迎语", PhraseBody.FromText("您好"), root.Id, ShortcutMode.None, null,
            0, null, 1, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow);
        var childPhrase = new Phrase(Guid.NewGuid(), "售前说明", PhraseBody.FromText("请问有什么可以帮您"), child.Id, ShortcutMode.None, null,
            0, null, 1, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow);
