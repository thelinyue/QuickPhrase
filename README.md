...
- Windows 11 x64。
- 企业微信 `5.0.9.6065`：固定使用受保护的 Clipboard + `Ctrl+V` 插入。
- 当前实际 WPF 界面包含话术库、编辑器、设置、分类管理和 Native Launcher。
- 自动发送、后台发送、图片/文件话术和云同步不属于 V1。
...
QuickPhrase 宁可不能发送，也不能发错窗口、发错内容或重复发送。目标变化、权限不一致、控件无法确认和 Clipboard 失败都会停止自动动作或降级为复制提示。插入或发送结果不确定时不自动重试。
