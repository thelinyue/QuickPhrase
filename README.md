...
- `Enter` 仅插入；`Ctrl+Enter` 是 Launcher 的通用显式发送操作。具体发送按键由 Adapter 决定，企业微信当前已验收配置使用 `Enter` 作为发送协议。后台发送、无用户授权自动发送、图片/文件话术和公共云同步不属于当前范围；用户可以选择连接自己配置的企业内网闪语中心，企业话术以只读缓存方式同步。
