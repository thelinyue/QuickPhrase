...
internal sealed class SearchService : ISearchService
{
    ...
    private SearchEntry BuildEntry(Phrase phrase, string? categoryPath, bool includePinyin)
    {
        ...
        return new SearchEntry(phrase, title, displayCategoryPath, Normalize(displayCategoryPath), Normalize(phrase.Content), full.ToImmutable(), initials.ToImmutable());
    }
    ...
    private static SearchMatchKind? FindStrongMatch(SearchEntry entry, string query)
    {
        ...
        if (entry.Category.Contains(query, StringComparison.Ordinal)) return SearchMatchKind.ContentContains;
