...
| `Ctrl+Enter` 显式发送 | 发布负责人确认当前通用 `InsertAndSend` 人工矩阵通过；旧实现“未发送”只保留为历史证据 | 通过；直接按段插入并触发发送快捷操作，已有草稿、焦点变化和异常中断均按安全边界验收 |
| 关闭全局兼容模式 | 发布负责人确认关闭后仍按受保护 Clipboard 路径执行 | 通过；未开放 Unicode 直输或后台发送 |

人工测试必须使用可控且允许发送测试消息的专用会话。执行 `Ctrl+Enter` 前必须确认目标与已有草稿，记录是否触发快捷操作及最终人工观察；QuickPhrase 不读取正文，也不能以自动化结果声明目标应用最终发送成功。出现不确定结果时停止，不重复动作。
